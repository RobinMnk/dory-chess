...
class CheckLogicHandler {
    template<bool, bool>
    static BB addPins(const BoardPtr& board, int kingSquare, bool& blockEP);

public:
    template<bool>
    static std::unique_ptr<PinData> reload(const BoardPtr& board);
};

template<bool whiteToMove, bool diag>
BB CheckLogicHandler::addPins(const BoardPtr& board, int kingSquare, bool& blockEP){
...
template<bool white>
std::unique_ptr<PinData> CheckLogicHandler::reload(const BoardPtr& board){
